...
//	print_r( $client->products->get() );
	...
	$jsonCat = file_get_contents('php://input');
    $category_info = json_decode($jsonCat);
    $category_slug = $category_info->category_slug;
    
   //$category_slug = "aromatic";

	
   echo json_encode($client->products->get(null,array('filter[category]'=>$category_slug) ), JSON_UNESCAPED_UNICODE);
